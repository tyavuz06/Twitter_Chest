<?php
	...
	{
		$result = mysqli_query($db_con, "select CountryID,CountryName from kcb_tbcountries where ContinentID=$selected_continent_id");
		while($row= mysqli_fetch_assoc($result))
		{
	...
	{
		$result = mysqli_query($db_con, "select CityID,CityName from kcb_tbcities where CountryID=$selected_country_id");
		while($row= mysqli_fetch_assoc($result))
		{
